...
public class MarkovChain {
    private final static String START_WORD = "(START)";
        ...
        while (map.containsKey(key)) {  // build the sentence
            ...
            if (list.size() > 1) {        // if multiple cases could follow, pick randomly according to frequency
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (randNum <= 0) {
                        next = list.get(i); break;  // pick the next word according to probability
                    }
                }
            }
    ...
    public static void main(String[] args) {
        try {
            String filename = args[0];  //String filename = "tao_te_ching.txt";
